...
`crop` は、正確なページスクリーンショットを撮るための Chrome Manifest V3
拡張機能です。オーバーレイを開き、ページ要素を選択するか任意の範囲を
ドラッグして、生成された PNG をコピーまたは保存できます。
...
![スクリーンショット](/assets/crop_screenshots/crop_jap_1.png)

## Chrome Web Store からインストール

1. [Chrome Web Store の `crop`](https://chromewebstore.google.com/detail/crop/pdmniipgbjdcpnhbkkppodechbehagki) を開きます。
2. Add to Chrome をクリックし、インストールを確認します。
3. 通常のウェブページを開き、`crop` 拡張機能アイコンをクリックします。

インストール後、推奨ショートカットが既存のブラウザーまたは OS のショートカットと競合する場合、Chrome はそのショートカットを未割り当てのままにすることがあります。拡張機能のショートカットは `chrome://extensions/shortcuts` で確認できます。
...
`crop` を開発する場合、ローカル変更をテストする場合、または未パッケージのビルドを手動で読み込む場合は、この手順を使います。
...
- ページ全体のキャプチャは `chrome.tabs.captureVisibleTab()` と scroll stitching を使います。stitched output がブラウザーの canvas 上限を超える場合、`crop` は単一画像として保存できるよう PNG を自動的に縮小します。lazy loading、animation、sticky layout の変化がある動的ページでは、引き続き不完全なキャプチャになることがあります。

...
`crop`은 Chrome MV3 확장으로, Firefox Screenshots와 유사한 요소 선택형 스크린샷 UX를 제공한다. MVP는 확장 아이콘/단축키 실행, Shadow DOM 기반 오버레이, DOM 요소 hover 하이라이트, 선택 영역 Copy/Save, visible viewport 캡처만 포함한다. Full page 캡처, scroll stitching, `debugger` 권한, `<all_urls>` 권한은 MVP 범위에서 제외한다.
...
- 제품명과 사용자-facing 브랜딩은 `crop`만 사용한다. Mozilla, Firefox, Screenshots를 제품명, 확장명, 아이콘, 제휴 암시 문구에 사용하지 않는다.
- Chrome MV3 MVP 권한은 `activeTab`, `scripting`, `clipboardWrite` 중심으로 유지하고, `debugger`와 `<all_urls>`는 별도 승인 없이 추가하지 않는다.
- Firefox 유래 코드는 `src/firefox-derived/`에 분리하고 MPL 2.0 헤더, `NOTICE`, `THIRD_PARTY.md` 출처를 유지한다.
- MVP 캡처 백엔드는 `chrome.tabs.captureVisibleTab()`의 visible viewport 기반으로 구현하며 full page/scroll stitching은 별도 task로 분리한다.
- overlay/하이라이트/버튼이 최종 PNG에 포함되지 않도록 capture 전 숨김 처리를 구현하고 검증한다.
...
- 현재 제품 계획 원본: `/Users/melee/Downloads/crop_development_plan_prompt.md` — 저장소 내 공식 제품 문서 위치는 별도 task에서 결정

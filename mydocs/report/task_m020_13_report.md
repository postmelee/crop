...
| `src/content/overlay/crop-template.ts` | selection controls, 8방향 resize handle, move surface, size badge DOM 추가 | Shadow DOM template |
| `src/content/overlay/crop-overlay.css` | resize handle, move surface, size badge, action box grouping/spacing/shadow/cursor 스타일 추가 | overlay visual/UI |
| `src/content/overlay/positioning.ts` | selection controls, size badge, action box viewport clamp/flip helper 보강 | viewport positioning |
...
| 하단 action box가 Firefox 원본에 가까운 크기, 색, grouping, border radius, shadow, 위치 규칙을 가진다 | OK — Stage 3 DOM/CSS와 positioning 테스트, Stage 5 README/smoke fixture 갱신으로 반영했다. |
...
- Pointer handle 체감 크기와 action box 시각 위치는 OS display scale, browser zoom, 페이지 density에 따라 조정 여지가 있을 수 있다.

...
| 경로 | 변경 요약 | 영향 범위 |
|---|---|---|
| `manifest.json` | MV3 source manifest, `crop` metadata, action, `open-crop` command, `activeTab`/`scripting`/`clipboardWrite` 권한 정의 | Chrome extension 로드 설정 |
...
| 사용자 shortcut smoke 후속 수정 | OK — 등록된 shortcut이 동작하지 않는 증상을 reserved `_execute_action` 의존 대신 `open-crop` custom command와 `commands.onCommand` 직접 처리로 수정했다. |

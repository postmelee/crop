...
| `tests/content/overlay/phase6-regression.test.ts` | selected capture runtime 분기, fixture marker, selected/full page sticky chrome suppression 정책 차이, 권한 경계를 회귀 테스트로 고정했다. | automated regression |
...
| 자동 테스트 | Stage 2 기준 185개 통과 | 최종 16개 파일, 187개 테스트 통과 |
...
| full page capture, visible viewport capture, iframe/shadow selection은 회귀하지 않는다 | OK — 전체 `npm test` 16개 파일 187개 테스트 통과 |
...
- OK: `npm test` 16개 파일, 187개 테스트 통과.
- OK: 권한 경계 grep 통과. `debugger`, `<all_urls>` 권한 추가 없음.
- OK: #26 핵심 경로 grep 통과. selected capture runtime, fixture, 품질 매트릭스, selected/full page suppression 정책 반영 확인.

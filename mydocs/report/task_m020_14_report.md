...
- 마일스톤: M020
- 단계 수: 4
- 작업 목적: Chrome MV3 권한 범위 안에서 same-origin/srcdoc iframe 내부 요소 선택을 지원하고, cross-origin iframe과 closed shadow DOM 제한을 안전하게 문서화한다.
...
| `tests/content/overlay/phase6-regression.test.ts` | overlay runtime의 iframe rect 우선 경로와 fixture iframe smoke target을 검증한다. | 통합 회귀 테스트 |
...
| `mydocs/working/task_m020_14_stage{1..4}.md` | 단계별 결과와 검증 근거를 기록했다. | 내부 작업 기록 |
| `mydocs/orders/20260531.md` | 오늘할일 상태를 Stage 4 + 최종 보고서 승인 대기로 갱신했다. | Hyper-Waterfall 추적 |
...
| `mydocs/working/task_m020_14_stage{N}.md` | `mydocs/working/` | `mydocs/working/task_m020_14_stage{1..4}.md` | OK | 단계 보고서 표준 위치와 일치 |
...
- 실제 Copy/Save 결과는 기존 정책대로 visible viewport intersection만 저장한다.

...
| `src/content/overlay/crop-overlay.ts` | overlay mount, 중복 실행 flash, hover helper 연결, 눈동자 pointer offset 갱신, click selection, Cancel/Escape teardown 구현 | overlay runtime controller |
| `src/content/overlay/crop-template.ts` | Shadow DOM template, top-right mode toolbar, 중앙 prompt/face, highlight, Copy/Save/Cancel buttons 생성 | overlay DOM 구조 |
| `src/content/overlay/crop-overlay.css` | dark dim layer, dashed viewport boundary, hover/selected highlight, prompt/face, action buttons, mode toolbar 스타일 | overlay UI styling |
| `src/content/overlay/state-machine.ts` | `idle`, `hovering`, `selected`, `closing` 상태와 전이 구현 | overlay 상태 모델 |
| `src/content/overlay/positioning.ts` | highlight placement, action buttons clamp/flip placement, eye offset helper 구현 | overlay layout helper |
...
| 지표 | 변경 전 | 변경 후 |
|---|---|---|
| content overlay entry | `src/content/inject.ts` inline stub 207줄 | `inject.ts` 3줄 bootstrap + `src/content/overlay/` 4개 TS module와 CSS 1118줄 |
| overlay 상태 모델 | 없음 | `state-machine.ts` 73줄, 6개 상태 전이 테스트 |
| overlay placement helper | 없음 | `positioning.ts` 152줄, 10개 placement/eye offset 테스트 |
| 전체 Vitest | 3개 test file, 25개 test | 5개 test file, 41개 test |
| 단계 보고서 | 없음 | Stage 1~3, Stage 5 보고서 4개, 총 290줄 |
| README smoke 기대 결과 | overlay stub 기준 | mode toolbar, 중앙 prompt/face, hover highlight, selected rectangle, Copy/Save/Cancel buttons 기준 |
...
| `npm run test` 통과 | OK — Vitest v3.2.4, 5개 test file, 41개 test 통과 |
...
| 중앙 preview prompt와 face 표시 | OK — CDP content-script smoke에서 안내 문구, Cancel button, face DOM, eye DOM 2개 확인 |
| 눈동자 pointer 추적 | OK — CDP content-script smoke에서 pointermove 후 `--crop-eye-x`, `--crop-eye-y` CSS custom property 갱신 확인 |
...
- Firefox 제품명, 원본 SVG path, 원본 아이콘 asset은 사용하지 않았다. 따라서 시각 구조와 동작은 Firefox식으로 맞췄지만 픽셀 단위 완전 복제는 의도적으로 제외했다.
...
- drag-to-region selection, resize handles, full page capture는 별도 이슈로 분리

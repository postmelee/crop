...
1. 중복 이슈 확인
   ```bash
   gh issue list --repo postmelee/crop --state all \
     --search "{작업 키워드}" \
     --limit 20 \
...
3. 기존 label 목록 확인
   ```bash
   gh api repos/postmelee/crop/labels --paginate \
...
8. 승인 후 이슈 생성
   ```bash
   gh issue create --repo postmelee/crop \
     --title "{제목}" \
     --body "{본문}" \
     --milestone "{milestone}" \
     --label "{label}"
...
9. 생성 결과 확인
   ```bash
   gh issue view {N} --repo postmelee/crop \
     --json number,title,state,milestone,labels,url
   ```

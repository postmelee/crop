...
- Store 제출용 privacy policy URL은 `main` 또는 release tag 기준 stable URL이어야 한다.
...
- `PRIVACY.md`가 `main` 반영 후 제출 URL 후보에 포함될 내용을 갖고 있다.
...
- `https://github.com/postmelee/crop/blob/main/PRIVACY.md`가 Dashboard에 제출할 privacy policy URL로 유효하다.
- tag URL을 privacy policy URL로 쓰려면 tag 생성 후 `https://github.com/postmelee/crop/blob/{tag}/PRIVACY.md`가 유효한지 확인한다.
- release note에는 Store 제출 ZIP 생성 명령과 검증 결과를 적는다.
...
- `main` 또는 release tag 기준 `PRIVACY.md` URL이 Dashboard에 입력됐다.
...
- `main` 또는 release tag 기준 `PRIVACY.md` URL이 아직 유효하지 않다.

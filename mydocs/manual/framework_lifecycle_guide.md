...
Hyper-Waterfall 버전 업데이트 PR은 release PR과 다르다. Release PR은 프레임워크 저장소의 `devel` 변경을 `main`로 승격해 tag 기준을 만드는 작업이고, Hyper-Waterfall 버전 업데이트 PR은 이미 적용된 저장소가 그 release/tag를 따라가도록 변경 후보를 리뷰하는 작업이다.

...
Hyper-Waterfall 방법론 자체의 canonical 배포 단위는 GitHub Release/tag다. `main`에 반영된 상태를 tag로 고정하고, 해당 release의 `templates/manifest.json`과 `docs/migrations/`를 기준으로 기존 적용 저장소가 업데이트한다.
...
릴리즈 시점에는 `devel`에서 검증된 변경을 `main`로 승격하는 PR을 만든다.

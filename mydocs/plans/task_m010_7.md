...
완료 후에는 사용자가 Chrome unpacked extension에서 영역을 선택한 뒤 `Copy`를 누르면 Firefox 원본처럼 overlay가 사라지고 우측 상단에 복사 완료 toast가 표시되어야 한다. `Save`는 별도 Chrome `downloads` 권한 없이 브라우저 기본 다운로드 흐름을 사용한다.
...
- Blob URL 또는 data URL 기반 `<a download>` Save 동작
...
- `chrome.downloads.download()` 기반 저장
- `downloads` 권한 추가
- full page capture
- scroll stitching
- `debugger` 권한
...
- Save는 `chrome.downloads` 권한 없이 `Blob` URL 또는 PNG data URL을 `<a download>`로 클릭하는 방식으로 구현한다. 생성한 object URL은 사용 후 revoke한다.
...
- Chrome MV3 MVP 권한 정책을 유지한다. `activeTab`, `scripting`, `clipboardWrite` 외 권한은 이번 task에서 추가하지 않는다.
...
- **Stage 3 — Save 다운로드 흐름**
  ...
  - sanitized filename과 object URL cleanup 검증
...
- `downloads`, `debugger`, `<all_urls>` 권한이 추가되지 않는다.
...
- **object URL leak**: Save에서 Blob URL을 사용할 경우 클릭 후 revoke를 보장한다.
...
- Save는 `chrome.downloads` 권한 없이 `<a download>` 기반으로 구현하는 것
- `downloads`, `debugger`, `<all_urls>` 권한을 추가하지 않는 것

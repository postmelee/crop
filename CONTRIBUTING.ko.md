...
언어: [English](CONTRIBUTING.md) | 한국어 | [简体中文](CONTRIBUTING.zh-CN.md) | [日本語](CONTRIBUTING.ja.md)

이 문서는 편의를 위한 한국어 번역입니다. 번역 간 의미가 다를 경우 English 문서가 기준입니다.
...
- Mozilla, Firefox, Screenshots 명칭은 출처, 라이선스, 기술 참고 맥락에서만 사용하고 제휴, 보증, 후원, 공식 제품처럼 보이게 쓰지 않습니다.
- Chrome MV3 권한은 좁게 유지합니다. `debugger`, `<all_urls>`, 광범위한 host permissions, `tabs` 권한은 maintainer와 사전 논의 없이 추가하지 않습니다.
- Firefox 유래 코드나 MPL 2.0 적용 파일을 다룰 때는 `src/firefox-derived/`, `NOTICE`, `THIRD_PARTY.md`, `LICENSE-MPL-2.0`의 경계와 고지를 유지합니다.
...
- 스크린샷은 브라우저 안에서 로컬로 처리된다는 privacy stance를 유지합니다.
- 서버 업로드, telemetry, 계정 동기화처럼 구현되지 않은 기능을 암시하지 않습니다.
- overlay, highlight, button이 최종 PNG에 포함되지 않도록 유지합니다.
- Chrome 제한 페이지, cross-origin iframe, closed shadow DOM, canvas 크기 제한 같은 제약은 명확히 설명합니다.
...
이 흐름은 maintainer와 작업 에이전트가 변경을 추적하기 위한 내부 운영 절차입니다. 외부 기여자는 일반적인 Issue와 Pull Request로 제안해도 됩니다. 필요하면 maintainer가 해당 제안을 추적 task로 전환하고 내부 workflow를 처리합니다.

## 리뷰와 merge

- 모든 PR은 maintainer review를 거칩니다.
- CI와 필요한 수동 검증이 통과해야 merge할 수 있습니다.
- 범위가 커지거나 정책 판단이 필요한 경우 maintainer가 작업을 Issue로 되돌려 다시 계획할 수 있습니다.
- Issue close는 maintainer가 승인하거나 관련 PR이 merge된 뒤 처리합니다.

## 질문과 논의

질문, 사용 후기, 기능 아이디어는 GitHub Issues 또는 Discussions를 사용합니다. 버그와 구체적인 작업 요청은 Issues가 더 적합하고, 넓은 제안이나 사용 경험 공유는 Discussions가 더 적합합니다.

...
1. [Chrome Web Store의 `crop`](https://chromewebstore.google.com/detail/crop/pdmniipgbjdcpnhbkkppodechbehagki)을 여세요.
2. Add to Chrome을 클릭하고 설치를 확인하세요.
3. 일반 웹 페이지를 열고 `crop` 확장 프로그램 아이콘을 클릭하세요.

설치 후 제안 단축키가 기존 브라우저 또는 운영체제 단축키와 충돌하면 Chrome이 단축키를 비워 둘 수 있습니다. 확장 단축키는 `chrome://extensions/shortcuts`에서 확인할 수 있습니다.
...
`crop`을 개발하거나, 로컬 변경을 테스트하거나, 압축 해제된 빌드를 수동으로 로드해야 할 때 이 경로를 사용하세요.
